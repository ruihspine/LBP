...
library(mapdata)
rm(list = ls())
df=read.csv("EAPC YLDs.csv",header = T)
load("./data/GBD.Rdata")
colnames(df)

labelx="Estimated annual percentage change in YLDs rate of LBP, 1990–2023"
dfx<- df %>%
...
ggplot(df_asr %>% na.omit()) +
  ...
  guides(fill = guide_legend(label.position = "bottom",
                             nrow = 1,
                             title.position = "top" ,title = labelx))->p 
...
ggsave("QN eapc YLDs map4.pdf",width = 12,height = 10)
